...
def rcLocalConfig(fileDir):
    config_rcLocal= """#!/bin/sh -e
#
# rc.local
#
...
""".format(fileDir)
    with open("/etc/rc.local", "w+") as file_rcLocal:
        file_rcLocal.write(config_rcLocal)
...
disable_splash=1
 """
    with open("/boot/config.txt", "w+") as file_bootConfig:
        file_bootConfig.write(config)
